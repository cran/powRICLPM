...
`summary()` can be used in one of four ways. First, summary can be used simply like `print()` to get information about the design of the power analysis (the different experimental conditions), as well as the number of problems the occurred per condition (e.g., non-convergence, fatal estimation errors, or inadmissible results). Second, by specifying the `parameter = "..."` argument in `summary()`, the function will print the results specifically for that parameter across all experimental conditions. Third, if you specify a specific experimental condition (by specifying a sample size, number of time points and ICC using the `sample_size`, `time_points`, and `ICC` arguments), the function displays performance measures for all parameters in that experimental condition. 
